...
- `tests/battle_ir/test_semantic_batch_catalog.py`
- `data/semantics/4.4.54/full_reconstruction/terra_implementation_master_v1.json`
...
- Derive expected artifact and primitive counts from the catalog objects exercised by each test.
- Keep an assertion that disabling the selected entry removes exactly that entry's primitive contribution.
- Do not replace 12/91/80 with 13/92/81 constants.
...
- Do not weaken hash validation, enabled filtering or primitive ID checks.
...
- `python -m unittest tests.battle_ir.test_semantic_batch_catalog -v`
- `python -m unittest discover -s tests/battle_ir -t . -p "test_*.py"; expected: green`
