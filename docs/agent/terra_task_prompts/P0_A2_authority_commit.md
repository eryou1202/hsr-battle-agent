# P0-A2 — Create controlled authority preservation commit
...
- `data/semantics/4.4.54/full_reconstruction/terra_implementation_master_v1.json` (task-local status/checkpoint fields only)
...
- Recompute every approved manifest hash before staging.
- Stage each approved file using a separate explicit `git add -- <path>` pathspec.
- Compare `git diff --cached --name-only` exactly with the approved stage list.
- Confirm both protected pre-existing modified tracked files remain unstaged.
- Create one preservation commit only after the manifest review flag is present.

## Commit authorization

This prompt authorizes only the reviewed preservation commit described by P0-A2. If the staged set differs by even one path, unstage only those explicit paths, record `BLOCKED`, and stop.

## Non-goals

- Do not modify file contents.
- Do not stage unrelated historical dumps or planning discoveries outside the approved manifest.
...
- `git diff --cached --name-only equals the approved manifest list before commit`
- `git diff --cached --check`
- `post-commit git show --name-only contains exactly the approved list`
- `post-commit hashes equal the manifest`
